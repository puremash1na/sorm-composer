...
final class Installer extends SormService
{
    ...
    CREATE TABLE IF NOT EXISTS logs_edit (
        ...
        recordId BIGINT NOT NULL COMMENT 'Идентификатор записи в соответствующей таблице (max 2^64-1)',
    ...
    private static function createTrigger(
        ...
        switch (strtoupper($operation)) {
            ...
            ";
                break;
            case 'DELETE':
                $jsonDataInfo = "JSON_OBJECT('$fieldString',OLD.".$fieldString.")";
                $sqlCreate = "
            ...
            foreach ($keys as $key => $value) {
                if (is_array($value)) {
                    foreach ($value as $subKey) {
                        ...
                        $template = str_replace("%{$subKey}%", "',$operation.$subKey,'", $template);
                    }
                } else {
                    if ($value === null || $value === '') {
                        continue;
